...
**Предметная база (ФРП):** для предметов из [`src/lib/knowledge/frp/manifest.json`](src/lib/knowledge/frp/manifest.json) (базовый уровень) сервер подбирает фрагмент рабочей программы по классу и теме и добавляет его в системный промпт. Обновление из PDF: `npm run extract:frp` (см. [`docs/README.md`](docs/README.md)). **Информатика 7–9:** если ФРП доступна, используется она; иначе — запасной файл [`informatika_7_9.md`](src/lib/knowledge/informatika_7_9.md). Имя основного файла методики — **`konstruktorUroka.md`** (регистр букв важен на Linux-серверах).

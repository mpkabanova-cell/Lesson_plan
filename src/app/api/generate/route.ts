...
function buildUserPayload(b: Body, fixBlock?: string): string {
  ...
  const parts = [
    ...
    "Ответы к проверяемым заданиям («Ответ:», «Разбор:») размещай сразу под соответствующим «Задание N.M» в том же этапе — внутри сценария, не в конце плана.",
    "Не создавай отдельный раздел **Ключи к заданиям** в конце сценария. Нумерация: «Задание N.M» — N = номер этапа в этом сценарии, M = номер задания внутри этапа.",
    "Любой эталон, текст, таблица или схема для сравнения/проверки должны быть напечатаны в том же этапе, где учитель на них ссылается. Запрещены формулировки «сравните с эталоном, который я вам дам» без самого эталона в сценарии.",

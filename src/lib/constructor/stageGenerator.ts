...
- На этапе домашнего задания пиши именно практические задания по теме и разобранному материалу: условия упражнений/примеров/вопросов + ответы или критерии. Не пиши вместо задания «запишите в дневник базовое/повышенное задание»
...
function buildStageUserPrompt(input: StageGenerationInput): string {
  ...
  const stageSpecificGuidance = (() => {
    ...
    if (input.stage.id === "homework_info") {
      return [
        "Дай конкретные практические задания по теме и разобранному на уроке материалу.",
        "Сформулируй сами условия заданий: базовый уровень и повышенный уровень, если это уместно.",
        "К каждому заданию добавь ответ, образец результата или чёткий критерий проверки.",
        "Не заменяй содержание ДЗ служебной фразой вроде «запишите в дневник базовое и повышенное задание».",
      ];
    }
    return undefined;
  })();

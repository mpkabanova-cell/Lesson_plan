...
export function motivationalStageOpeningSpeech(input: {
  ...
  if (input.lessonType === "new_knowledge") {
    const hook =
      input.hook?.trim() ||
      `Посмотрите на первый пример по теме «${input.topic}»: что здесь кажется непонятным или неожиданным?`;
    return `«Здравствуйте! Сегодня мы начнём изучать новую тему «${input.topic}» ${meta}. Проверьте, пожалуйста, готовность: учебник, тетрадь, ручка. ${hook} Запишите одну версию ответа — к ней вернёмся в конце урока.»`;
  }
